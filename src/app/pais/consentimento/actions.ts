...
export async function acceptConsentAction(formData: FormData) {
  ...
  const signatureUrl = await uploadFile(immutableSignatureKey("consents", acceptance.guardianId, acceptance.id, buffer), buffer, "image/png");
  ...
  const accepted = await prisma.consentAcceptance.updateMany({
