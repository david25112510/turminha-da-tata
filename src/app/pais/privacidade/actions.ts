...
export async function acceptPrivacyPolicyAction(formData: FormData) {
  ...
  if (!agreed) throw new Error("É preciso marcar que você leu e compreendeu a política.");
  ...
  if (!acceptance) throw new Error("Política de Privacidade não encontrada.");
  ...
  await recordAuditLog({
    ...
    action: "POLITICA_PRIVACIDADE_ASSINADA",
  ...
  await recordAuditLog({
    actorUserId: session.user.id,
    action: "POLITICA_PRIVACIDADE_ACEITA",
    entity: "PrivacyPolicyAcceptance",
    entityId: acceptance.id,
    newData: { versionId: acceptance.versionId },
  });
