...
import { PrivacyPolicyAcceptanceCard } from "./PrivacyPolicyAcceptanceCard";

export const dynamic = "force-dynamic";

/**
 * Fora do route group (portal), mesmo motivo de src/app/pais/contrato/page.tsx e
 * src/app/pais/consentimento/page.tsx: não herda o layout que bloqueia acesso quando há
 * pendência, então continua acessível independente do status. Verificada por último no layout
 * (depois de contrato e consentimento) — se todos estiverem pendentes, esta tela só aparece
 * depois que os outros dois forem aceitos.
 */
export default async function PrivacyPolicyAcceptancePage() {
  const guardian = await requireGuardian();

  const pending = await prisma.privacyPolicyAcceptance.findFirst({
      ...
      <main className="flex-1 p-4 sm:p-6 flex flex-col gap-5 max-w-2xl mx-auto w-full">
        <div>
          ...
          <p className="text-sm text-tata-ink-muted-alt">
            Antes de continuar no Portal dos Pais, leia e confirme a Política de Privacidade.
          </p>
        </div>
        ...
        <PrivacyPolicyAcceptanceCard
          ...
          action={acceptPrivacyPolicyAction}
        />
