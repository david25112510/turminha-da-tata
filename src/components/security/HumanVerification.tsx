...
export function HumanVerification({
  ...
  const widgetId = useRef<string | null>(null); const wasPending = useRef(false); const [loaded, setLoaded] = useState(false); const [loadFailed, setLoadFailed] = useState(false);
  ...
  }, [id, loaded, onVerifiedChange, siteKey]);
  useEffect(() => { if (wasPending.current && !pending && widgetId.current && window.turnstile) { onVerifiedChange?.(false); window.turnstile.reset(widgetId.current); } wasPending.current = pending; }, [onVerifiedChange, pending]);
  return <div className="flex min-h-[70px] flex-col justify-center" aria-label="Verificação de segurança"><Script src="https://challenges.cloudflare.com/turnstile/v0/api.js?render=explicit" strategy="afterInteractive" onReady={() => setLoaded(true)} onError={() => setLoadFailed(true)} />{siteKey && !loadFailed ? <div id={id} /> : <p role="alert" className="text-sm text-tata-coral-dark">Verificação de segurança indisponível.</p>}</div>;
}
