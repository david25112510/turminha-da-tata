...
`EnrollmentRequest` separa o portal público do domínio operacional. A conta `User/Guardian` usa o NextAuth existente, mas solicitações pendentes não criam `Child`. Na aprovação, uma transação reivindica a solicitação, verifica duplicidade e cria/reutiliza criança, vínculo, autorizados e aceites documentais. `approvedChildId` e a mudança condicional de status tornam reenvios idempotentes. Server Actions revalidam RBAC e decisões geram auditoria e notificações.
...
### Cadastros
- **Child** — dados da criança **e** as configurações financeiras/de horário (`contractedEntryTime`, `contractedExitTime`, `contractedDays`, `toleranceMinutes`, `monthlyFee`, `overtimeHourRate`, `dueDay`) e as 5 autorizações de imagem por finalidade (`imageAuthInternal`, `imageAuthGuardianShare`, `imageAuthInstitutional`, `imageAuthSocialMedia`, `imageAuthAdvertising`).
...
- **GuardianInvite**/**SignupRequest** — auto-cadastro com aprovação (ver "Autenticação e controle de acesso" abaixo para o fluxo completo).
...
### Exclusão permanente (crianças, responsáveis, cuidadoras)
Admin pode excluir de verdade (não só desativar) em `/admin/criancas/[id]`, `/admin/responsaveis` e `/admin/cuidadoras/[id]` — `DeleteConfirmDialog` (`src/components/tata/DeleteConfirmDialog.tsx`) exige digitar o nome exato antes de habilitar o envio, e cada Server Action grava `AuditLog` (`action: "DELETE"`, com o snapshot em `oldData`) **antes** de apagar, nunca silenciosamente.

Regra aplicada de forma consistente em todo `prisma/schema.prisma`: **dado que pertence à CRIANÇA nunca é apagado pela exclusão de uma pessoa** — presença, rotina (refeição/sono/higiene/água/humor/atividade/saúde), medicamentos (autorização e administração), ocorrências, fotos, `AuditLog`, faturas e pagamentos usam `onDelete: SetNull` nas FKs para `User`/`Guardian` (campo tornado opcional onde precisou). Excluir uma cuidadora ou um responsável só remove a referência de "quem fez" — o registro em si permanece, igual a como o GitHub trata comentários de uma conta apagada. Já vínculos/cadastros que pertencem à PRÓPRIA pessoa (`GuardianChild`, `AuthorizedPickupPerson` cadastrada por ela, aceites de contrato/consentimento, notificações, push) usam `onDelete: Cascade` e somem junto.

Excluir uma **criança** é o único caso sem essa proteção — cascade total (decisão explícita, sem retenção): apaga todo o histórico dela, incluindo financeiro já pago. Excluir um **responsável** também remove a conta de acesso ao portal vinculada (`Guardian.userId`), numa transação. Excluir uma **cuidadora** não tem esse efeito colateral em outras crianças — ver o parágrafo anterior.
...
### Consentimento LGPD
- **ConsentVersion**/**ConsentAcceptance** — mesmo desenho do contrato digital acima (versão publicada, aceite com assinatura/hash/IP/userAgent), mas por **responsável**, não por par criança/responsável (`@@unique([guardianId, versionId])`) — o consentimento é sobre o tratamento dos dados pessoais do próprio titular, não faz sentido duplicar por filho.

### Política de Privacidade
- **PrivacyPolicyVersion**/**PrivacyPolicyAcceptance** (`src/lib/privacy-policy.ts`, `src/app/pais/privacidade/`) — mesmo desenho do Consentimento LGPD (por responsável, assinatura manuscrita completa via o mesmo `DocumentAcceptanceWizard`), documento separado por ser uma finalidade legal distinta: descreve como a instituição trata dados em geral, não é o aceite pontual de uma finalidade específica. Bootstrap preguiçoso da v1.0 igual aos outros dois (`getCurrentPrivacyPolicyVersion`). Bloqueio do portal e ordem de checagem: contrato → consentimento → política de privacidade (`src/app/pais/(portal)/layout.tsx`), uma pendência de cada vez. `/pais/documentos` lista os 3 documentos aceitos (contrato por criança, política de privacidade por responsável); consentimento ainda não tem visualizador próprio ali. Distinto do contrato de prestação de serviços, do consentimento LGPD e de `Child.imageAuthX` — ver `docs/lgpd.md` para a finalidade de cada um dos quatro.
...
- **Auto-cadastro com aprovação** (`/cadastro`, público, fora de qualquer prefixo protegido): a pessoa escolhe CUIDADORA ou RESPONSÁVEL (nunca ADMIN) e envia `requestSignupAction` (`src/app/cadastro/actions.ts`) — isso **nunca cria um `User`**, só um `SignupRequest` (`status: PENDING`), com a senha já hasheada. Rate limit por e-mail reaproveitando `src/lib/rate-limit.ts`. RESPONSÁVEL exige um **código de convite** vinculado a uma criança específica (`GuardianInvite`, gerado pelo admin na ficha da criança e repassado pessoalmente à família — nunca um link público; só o hash do código é persistido, mesmo padrão de `PasswordResetToken`) — sem ele, é impossível se auto-vincular a uma criança. O código é consumido atomicamente (`updateMany` condicional em `src/lib/guardian-invite.ts`, evita corrida entre duas tentativas simultâneas) já no momento da solicitação, não só na aprovação; se a solicitação for recusada, o código fica queimado e o admin gera outro. Só em `/admin/solicitacoes` (`approveSignupRequestAction`) uma conta é criada de verdade — para GUARDIAN, cria também `Guardian`+`GuardianChild` (permissões básicas por padrão: `viewRoutine`/`viewPhotos`/`receiveNotifications`/`receiveCommunications`, mas nunca `viewFinancial`/`authorizePickup`/`authorizeMedication` — essas ficam para o admin liberar manualmente depois) e os aceites de contrato/consentimento pendentes, reaproveitando `ensureContractAcceptance`/`ensureConsentAcceptance` (mesmo caminho de `createGuardianAction`).
...
- **Bloqueio por documento pendente**: `src/app/pais/(portal)/` é um *route group* do Next — não aparece na URL (`(portal)/jornada/page.tsx` continua servindo `/pais/jornada`), mas isola o `layout.tsx` que checa, em ordem, `ContractAcceptance` → `ConsentAcceptance` → `PrivacyPolicyAcceptance` pendentes, redirecionando para `/pais/contrato`, `/pais/consentimento` ou `/pais/privacidade` respectivamente — uma pendência de cada vez. Nenhuma dessas 3 rotas fica dentro do grupo, então nenhuma herda esse layout — por isso nunca entra em loop de redirecionamento e cada uma continua acessível mesmo com as outras pendentes.
- Autorização de imagem tem 5 categorias independentes em `Child` (`imageAuthInternal`, `imageAuthGuardianShare`, `imageAuthInstitutional`, `imageAuthSocialMedia`, `imageAuthAdvertising`), gerenciadas só pelo admin. `imageAuthInternal` é checada no upload (`src/lib/photo-actions.ts` recusa se a criança não tiver essa categoria, e exige papel `ADMIN`/`CAREGIVER`); `imageAuthGuardianShare` é checada na exibição ao responsável (`/pais/fotos` e a jornada só mostram fotos se `viewPhotos && imageAuthGuardianShare`). As outras 3 categorias (institucional, redes sociais, publicidade) são capturadas para fins de conformidade/consentimento, mas hoje não gateiam nenhum fluxo do próprio app (não há integração com redes sociais nem geração de material publicitário) — ver `docs/lgpd.md`.
...
| Público | `/cadastro` | Auto-cadastro de cuidadora/responsável — nunca cria conta direto, gera `SignupRequest` pendente de aprovação |
...
| Admin | `/admin/solicitacoes` | Fila de aprovação do auto-cadastro (`PENDING`/`APPROVED`/`REJECTED`) — só aqui uma conta é criada de verdade |
...
- **Uploads**: `src/lib/storage.ts` persiste referências opacas `storage://...` e envia para bucket S3 privado quando configurado; sem S3, usa `.data/uploads/` atrás de uma rota autenticada. Disco local não serve para múltiplas instâncias ou filesystem efêmero — detalhado em `deploy.md`.
...
- **Fotos legadas**: registros criados antes da migração podem ainda conter URLs públicas permanentes. Novos uploads usam referências privadas; migre ou remova objetos legados no provedor para eliminar essa exposição histórica.
- **Correções de rotina**: registros elegíveis podem ter o horário corrigido com motivo obrigatório e `AuditLog` contendo antes/depois. Medicamentos e incidentes permanecem imutáveis; uma retificação deve ser registrada como novo evento/adendo.
- **Financeiro**: a listagem mensal usa agregação em lote com quantidade fixa de consultas, evitando uma chamada por criança.
- **Fotos de rotina/perfil validam o tipo real do arquivo pelos magic bytes** (`src/lib/file-validation.ts`), não mais só por `file.type` (que é só uma sugestão do cliente, forjável). Geração de convite de responsável (`generateGuardianInviteAction`) ganhou rate limiting (5/15min por admin); upload de fotos deliberadamente **não** recebeu o mesmo limite — é uma ação legitimamente frequente durante o registro de rotina, e o mesmo limite ali seria regressão de UX, não proteção real.
