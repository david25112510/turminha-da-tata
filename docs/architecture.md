...
- **Não há edição retroativa dos registros de rotina** (refeição, sono, higiene, água, humor, atividade) — se a cuidadora registrar um horário errado, a única correção possível hoje é um novo registro (ou uma observação explicando o engano), sem um fluxo de "editar com motivo, preservando oldData/newData" dedicado a cada tipo de evento. Caberia como extensão futura, uma tela de edição por tipo.
- **`admin/financeiro/page.tsx` tem um N+1 conhecido** (`getMonthlyOvertimeBreakdown` chamado por criança dentro de um `Promise.all`) — não corrigido deliberadamente nesta rodada: volume de uma creche não sente o efeito, e a função tem cobertura de teste extensa que uma refatoração arriscaria sem necessidade real.
