...
- `src/auth.ts` configura o Credentials provider: `authorize()` busca o `User` por e-mail, confere `active` e o hash da senha, e retorna `{ id, email, name, role }`. A sessão é JWT; `role` e `id` são propagados no callback `jwt`/`session` e tipados em `src/types/next-auth.d.ts`.
- `src/proxy.ts` (equivalente ao antigo `middleware.ts`) mapeia prefixo de rota → papel exigido (`/admin` → `ADMIN`, `/cuidadora` → `CAREGIVER`, `/pais` → `GUARDIAN`). Sem sessão, redireciona para `/login?callbackUrl=...`; com papel errado, redireciona para `/login`.
- Dentro do portal dos pais, o controle de acesso é **duas camadas**: o proxy garante que só um `GUARDIAN` logado entra em `/pais/*`; cada página então usa `src/lib/guardian.ts` (`requireGuardian()`) para carregar os vínculos `GuardianChild` do usuário e checa a permissão específica daquela seção (ex.: `link.viewFinancial`) antes de renderizar dado sensível.
- Autorização de imagem (`Child.imageAuthorized`) é checada tanto no upload (`src/lib/photo-actions.ts` recusa se a criança não tiver autorização) quanto na exibição (`/pais/fotos` e a jornada só mostram fotos se `viewPhotos && imageAuthorized`).
