...
- **Autorização de imagem é um único booleano** (`Child.imageAuthorized`): cobre uso interno, compartilhamento com o responsável, comunicação institucional, redes sociais e publicidade com o mesmo "sim". Uma autorização geral hoje libera todos esses usos de uma vez — separar em categorias independentes exigiria migração de schema (`imageAuthorized: Boolean` → registro por categoria) e é uma decisão que precisa de confirmação antes de implementar.
